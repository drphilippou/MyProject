...
#import "OBRRouteListVC.h"

@interface OBRRouteListVC () {
    UITableView* tvp;
...
-(void)viewDidAppear:(BOOL)animated {
    //start updating the bus data
    db.updateVehicles = true;
    
    //set the guidance flag to prevent database updates
    db.guiding = true;
}
...
-(void)refreshTableData {
    ...
    bool newdata = [db solvedRoutesModified];
    if (newdata) {
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(MyCustomCellTableViewCell*)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (r == selectedRoute) {
        cell.contentView.backgroundColor = LIGHT_YELLOW;
    } else {
    ...
    for (OBRsolvedRouteRecord* rec in ra) {
        ...
        if (rec.isRoute) {
            if (timeLinePtr+1 < ra.count) {
                ...
                iv.hidden = false;
                IOSImage* bwimage = [IOSI IOSimageWithFilename:@"DataHollow.png"];
                if (rec.busNum>0) {
                    ...
                    [bwimage drawText:rec.route atPoint:spacing FontSize:40];
                    iv.image = [bwimage colorize:busColor];
                    
                } else {
                    iv.image = [bwimage drawText:rec.route atPoint:spacing FontSize:40];
                }
    ...
    if ([cell.departureLabel respondsToSelector:@selector(setAttributedText:)]) {
        ...
        if (RTDAfirstBus) {
            if (firstAd>=0) {
                [attributedText setAttributes:@{NSForegroundColorAttributeName:FOREST_GREEN,
        ...
        if (RTDAlastBus) {
            if (lastAd>=0) {
                [dat setAttributes:@{NSForegroundColorAttributeName:FOREST_GREEN,
                                     NSFontAttributeName:[UIFont boldSystemFontOfSize:20]} range:(NSMakeRange(0, dat.length))];
            } else {
...
-(NSMutableAttributedString*)adherenceStr:(long)a label:(UILabel*)l{
    ...
    if (a==0) {
        ...
        NSMutableAttributedString *at = [[NSMutableAttributedString alloc] initWithString:@"(On Time)"
                                                                               attributes:@{NSForegroundColorAttributeName:FOREST_GREEN,
